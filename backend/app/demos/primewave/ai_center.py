"""
AI-Center Playground history (Primewave demo) — persistent ledger of
free-form vision questions the operator asked over a Frigate camera
snapshot OR an uploaded image, plus the model's answer.

Identical schema to `demos.restaurant.ai_center` so the restaurant
playground UI ports across as-is — only the on-disk storage path is
namespaced to this demo:
  - JSON ledger at `data/demos/primewave/ai_center.json`
  - Thumbnails at `data/demos/primewave/ai_center/<id>.jpg`

Both are GITIGNORED (operator-supplied images can be PII).
"""
...
logger = logging.getLogger("demo_primewave.ai_center")
